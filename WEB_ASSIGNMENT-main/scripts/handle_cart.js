...
function generateItem(id, name, image_url, price, short_description, quantity) {
  ...
  product_template.className =
    'col-lg-10 col-sm-12 card border-0 border-bottom border-dark rounded-0 bg-light mb-3 pe-0';
...
async function update_cart() {
  ...
  // Others
  let subtotal_value = 0.0;
  let estimated_tax_value = 0.0;
  let shipping_value = 0.0;
  let total = 0;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  product_container.addEventListener('click', async (event) => {
    ...
    if (
      clickedElement.id.match(btn_update) ||
      clickedElement.id.match(btn_update_svg) ||
      clickedElement.id.match(btn_update_path)
    ) {
      let this_child_id = clickedElement.id.match(/\d+/);
      this_child_id = parseInt(this_child_id[0]);

      let current_quantity = get_label_quantity(this_child_id);
      let updating_Mycart = await update_current_cart(
        myToken,
        this_child_id,
        current_quantity
      );
